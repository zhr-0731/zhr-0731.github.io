...
def update_rss_feed(new_articles):
    ...
    for article in new_articles:
        if article['link'] not in existing_links:
            ...
            # 添加全文内容 (content:encoded)，使用 CDATA
            if article['content']:
    ...
    # 如果需要将新文章插入到顶部（最新的在前），可以调整顺序
    # 但这里保持追加顺序，因为阅读器通常按 pubDate 排序
    # 如果你希望最新的在前，可以在这里对 new_items 进行 reverse 然后插入到 channel 的开头
